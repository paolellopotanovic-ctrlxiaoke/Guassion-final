...
class StateUpdate(pt.nn.Module):
    def __init__(self, Ns, Nh, Nk, edge_attr_dim=0):
        ...
        self.edge_input_dim = 6 * Ns + 1 + self.edge_attr_dim
        ...
        self.eqkm = pt.nn.Sequential(
            pt.nn.Linear(self.edge_input_dim, Ns),
            pt.nn.ELU(),
            pt.nn.Linear(Ns, Ns),
            pt.nn.ELU(),
            pt.nn.Linear(Ns, Nk),
        ...
        self.evm = pt.nn.Sequential(
            pt.nn.Linear(self.edge_input_dim, 2*Ns),
            pt.nn.ELU(),
    ...
    def forward(self, q, p, q_nn, p_nn, d_nn, r_nn, edge_attr_nn=None):
        # q: [N, S]
        # p: [N, 3, S]
        # q_nn: [N, n, S]
        # p_nn: [N, n, 3, S]
        ...
        if self.edge_attr_dim:
            if edge_attr_nn is None:
                edge_attr_nn = pt.zeros((N, n, self.edge_attr_dim), device=q.device, dtype=q.dtype)
            X_e = pt.cat([X_e, edge_attr_nn.to(device=q.device, dtype=q.dtype)], dim=2)
...
class StateUpdateLayer(pt.nn.Module):
    def __init__(self, layer_params, edge_attr_dim=0):
        super(StateUpdateLayer, self).__init__()
        # define operation
        self.edge_attr_dim = int(edge_attr_dim)
        self.su = StateUpdate(*[layer_params[k] for k in ['Ns', 'Nh', 'Nk']], edge_attr_dim=self.edge_attr_dim)
    ...
    def forward(self, Z):
        # unpack input
        if len(Z) == 6:
            q, p, ids_topk, D_topk, R_topk, edge_attr_topk = Z
        else:
        ...
        edge_attr_nn = edge_attr_topk[:, self.m_nn] if edge_attr_topk is not None else None
        # q, p = self.su.forward(q, p, q[ids_nn], p[ids_nn], D_topk[:,self.m_nn], R_topk[:,self.m_nn])

        if os.environ.get("PESTO_DISABLE_GRAD_CHECKPOINT", "0") == "1":
            if edge_attr_nn is None:
                q, p = self.su.forward(q, p, q[ids_nn], p[ids_nn], D_topk[:,self.m_nn], R_topk[:,self.m_nn])
            else:
                q, p = self.su.forward(q, p, q[ids_nn], p[ids_nn], D_topk[:,self.m_nn], R_topk[:,self.m_nn], edge_attr_nn)
        else:
            # with checkpoint
            q = q.requires_grad_()
            p = p.requires_grad_()
            if edge_attr_nn is None:
                ...
                q, p = checkpoint(self.su.forward, q, p, q[ids_nn], p[ids_nn], D_topk[:,self.m_nn], R_topk[:,self.m_nn], edge_attr_nn)

        # sink
        q[0] = q[0] * 0.0
        p[0] = p[0] * 0.0

        if edge_attr_topk is not None:
            return q, p, ids_topk, D_topk, R_topk, edge_attr_topk
